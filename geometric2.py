...
# print(sasa_values, sasa_values.shape)
# exposed_coords = []
# exposed_radii = []
...
# print(np.array(exposed_coords), len(exposed_coords))
# print(exposed_radii, len(exposed_radii))

# print('--'*30)

sasa_values = np.array(mol.sasaList())
print(sasa_values, sasa_values.shape)
exposed_coords = []
exposed_radii = []
for s, c, r, sas in zip(sasa_values, coords, radii, sasa_values):
    if sas >= 0:
        exposed_coords.append(c.tolist())
        exposed_radii.append(r+1.4)
print(np.array(exposed_coords), len(exposed_coords))
print(np.array(exposed_radii), len(exposed_radii))
...
mask = tree.query(surface_grid)[0] <= radii[tree.query(surface_grid)[1]]+1.4
print(mask)
pocket_points = []
for ijk, m in zip(surface_grid, mask):
    ...
    for angle in angles_combination:
        ...
        collisions = tree.query(points)[0] <= radii[tree.query(points)[1]]+1.4
        # print(tree.query(points)[0], radii[tree.query(points)[1]])
        # print(collisions)
