...
# Extract the 3D positions from the solutions_probability dictionary
positions = []
for residue in solutions_probability.values():
...
# Perform DBSCAN clustering with a distance threshold of 5 angstroms
dbscan = DBSCAN(eps=5, min_samples=3)
labels = dbscan.fit_predict(positions)

# Print the number of clusters and their sizes
unique_labels = set(labels)
num_clusters = len(unique_labels) - (1 if -1 in labels else 0) # ignore noise points
